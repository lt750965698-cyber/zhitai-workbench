...
- 固定命令的服务管理、来源白名单、路径边界、可选的签名兼容校验，以及公开发布和部分服务控制的确认门；
...
- 收件入口保留可选签名兼容模式：只有客户端携带签名且本机配置了密钥时，才校验 HMAC、时间窗和 nonce。它不是强制认证，也不能防御同一用户下的恶意本地进程。
...
仓库包含一个需要界面确认的可选模块更新器，它会从已配置的 GitHub 上游下载源码、DMG 或更新文件到本机私有 runtime。它不代表第三方组件已捆绑进织台发行物，也不提供通用的发布者签名或加密校验和验证；启用前仍应由使用者核对固定版本、许可证与上游资产。MatrixMedia 的本地安装流程还会修改应用的后台运行标记并进行 ad-hoc 重签名，修改后的副本不再等同于上游原始签名产物。
