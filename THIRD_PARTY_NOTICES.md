...
这不表示第三方组件被捆绑进仓库或默认发行物，但下载、修改、安装和组合使用仍受上游许可证约束。更新器把请求绑定到精确 GitHub owner、仓库、Release tag 与文件路径；二进制 Release 资产和热更新清单必须通过 GitHub 提供的 SHA-256，热更新清单内的每个文件也必须提供并通过 SHA-256，否则拒绝覆盖。GitHub 自动生成的源码归档仍没有统一 Release digest，项目也没有为所有组件验证发布者签名。
